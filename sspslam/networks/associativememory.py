...
class AssociativeMemory(nengo.network.Network):
    def __init__(self, n_neurons, d_key, d_value,
                 ...
                 scaling_factor=1, stable=True,encoders=None,radius=1,**kwargs):
        ...
        with self:
            ...
            self.recall = nengo.Node(size_in=d_value, label='memory_recall')
            ...
            self.conn_in = nengo.Connection(self.key_input, self.memory, synapse=None, learning_rule_type=voja)
            ...
            self.error = nengo.Ensemble(n_neurons, d_value)
            nengo.Connection(
                self.learning, self.error.neurons, transform=[[-2.5]] * n_neurons, synapse=None
            )
        
            # Calculate the error and use it to drive the PES rule
            nengo.Connection(self.value_input, self.error, transform=-1, synapse=None)
            nengo.Connection(self.recall, self.error, synapse=None)
