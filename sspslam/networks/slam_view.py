...
class SLAMViewNetwork(nengo.network.Network):
    ...
    def __init__(self, ssp_space, lm_space, view_rad, n_landmarks,
        ...
        with self:
            ...
            self.view_input = nengo.Node(size_in=d, label='lm_input')
            ...
            nengo.Connection(self.view_input, self.assomemory.key_input, synapse=None)
...
def get_slamview_input_functions(ssp_space, lm_space, velocity_data, vec_to_landmarks_data, view_rad, dt=0.001):
    r""" A helper function. 
   ...
   # At time t, if a landmark is in view return the Semantic Pointer representation of the landmark
    ...
    def landmark_func(t):
        ...
        for cur_id in cur_ids:
            result += ssp_space.bind(landmark_sps[cur_id], 
                       ssp_space.encode(vec_to_landmarks_data[int((t-dt)/dt), cur_id, :]).flatten()).flatten()
    ...
    return velocity_func, vel_scaling_factor, is_landmark_in_view, landmark_func
